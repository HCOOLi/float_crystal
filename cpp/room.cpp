...
void Room::inputECC(int num, int length) {  //
    ...
    for (int i = 0; i < sqrt_num; i++) {
        for (int j = 0; j < sqrt_num; j++) {
            // int type = rand() % 2 + 1;
            if (i * sqrt_num + j < num) {
                vec init{start_point[0] + i, start_point[1] + j,
                         start_point[2]};
...
int Room::cal_crystallinity(int q) const {
    ...
    for (auto &p : polymer_list) {
        ...
        for (int j = 1; j < length; j++) {
            if ((this->*count_parallel)(p[j - 1].location, p[j].location, a,
                                        1) > q) {
                num++;
            }
        }
    ...
    while (getline(filestream, temp)) {
        if (temp[0] == '#') {
            ...
            if (pos != -1) {
                ...
                if (result.size() == 3) {
#ifdef _WIN32
                    sscanf_s(data_str.c_str(), "%d%d%d", &this->shape[0],
            ...
            if (pos != -1) {
                ...
                if (result.size() == 9) {
                    ...
                    sscanf_s(data_str.c_str(), "%lf%lf%lf%lf",
                             &this->Ep_matrix[0][0], &this->Ep_matrix[0][1],
...
double Room::cal_Ec() const {
    ...
    for (const auto &p : polymer_list) {
        ...
        for (int j = 2; j < length; j++) {
            num +=
                cal_ifline(p[j - 2].location, p[j - 1].location, p[j].location);
        }
    }
    return -num;
}
...
double Room::cal_dEe2e_nearby(stack<Position> path) {
    ...
    if ((p1->pos_in_chain) > (p2->pos_in_chain)) {
        for (int i = p1->pos_in_chain + 1, j = 0; i < length && j < 1;
             i++, j++) {
        ...
        for (int i = p2->pos_in_chain + 1, j = 0; i < length && j < 1;
             i++, j++) {
            new_path.push_back(polymer_list[chain_num][i].location);
            old_path.push_back(polymer_list[chain_num][i].location);
        }
...
double Room::cal_dEc_nearby(stack<Position> path) const {
    ...
    int length = polymer_list[chain_num].chain.size();
    if ((p1->pos_in_chain) > (p2->pos_in_chain)) {
        for (int i = p1->pos_in_chain + 1, j = 0; i < length && j < 2;
             i++, j++) {
            new_path.push_front(polymer_list[chain_num][i].location);
            old_path.push_front(polymer_list[chain_num][i].location);
        }
        ...
        for (int i = p2->pos_in_chain + 1, j = 0; i < length && j < 2;
             i++, j++) {
            new_path.push_back(polymer_list[chain_num][i].location);
            old_path.push_back(polymer_list[chain_num][i].location);
        }
...
double Room::cal_Ep() const {
    ...
    for (const auto &p : polymer_list) {
        ...
        for (int j = 1; j < length; j++) {
            num +=
                (this->*count_parallel)(p[j - 1].location, p[j].location, a, 1);
        }
    }
    return num / 2.0;
}
...
double Room::cal_one_Ep(int i) const {
    ...
    int length = polymer.chain.size();
    for (int j = 1; j < length; j++) {
        num += (this->*count_parallel)(polymer[j - 1].location,
                                       polymer[j].location, a, 0);
    }
    return num;
}
...
#ifdef TRUE_POSITION
                if (find_in_que(que,
                                make_pair(p1, lattice[p1]->true_position)) &&
