...
#include<stdio.h>
template<typename T>
bool find_in_que(const deque<T> &que,const T& p) {
...
void Room::save(string filename) {
    ofstream file(filename, ios::out | ios::trunc);// ¸²¸Ç
...
void Room::load(string filename) {
    ...
    while (getline(file, temp)) {
        ...
        int x, y, z, t, m, t_p;
        sscanf(temp.c_str(), "%d%d%d%d%d%d", &x, &y, &z, &t, &m, &t_p);
