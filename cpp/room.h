...
class Room {
 public:
  ...
  double (Room::*count_parallel)(const vec &, const vec &, deque<Position> &,
                                 int) const {};

  double (Room::*cal_Eb_func)(const vec &, const vec &, deque<Position> &,
                              int) const {};

  vector<Polymer> polymer_list;

  Polymer &get_polymer(int i) { return polymer_list[i]; }

  Room() = default;
  Room(int x, int y, int z, int type = 24);
  Room(int x, int y, int z, vector<vector<double>> Ep,
  ...
  double cal_dEf(stack<Position> path);
  ...
  // deque<vec>& que, int cal_type)const; 	double count_parallel_B(vec &
  // point1,
