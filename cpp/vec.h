...
inline vector<double> mean(const vec &a, const vec &b) {
    return vector<double>{(a[0] + b[0]) / 2.0, (a[1] + b[1]) / 2.0,
                          (a[2] + b[2]) / 2.0};
}
inline double mean(const vec &a) { return 0; }
...
class Matrix3 {
   ...
   public:
    ...
    vector<vector<int>> &operator[](int x) { return _v[x]; }
    const vector<vector<int>> &operator[](int x) const { return _v[x]; }
};